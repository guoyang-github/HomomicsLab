...
> **v0.5.0** — 意图驱动的分析自动化，支持单文件领域声明、CLI 脚手架、LLM 辅助领域生成、运行时热加载、动态智能体角色、多智能体集群、自我进化的 SkillDAG、动态重规划、CBKB 策展、多层稳定性保障、可复现包、大数据结果卸载、技能记忆化、CodeAct 缓存、跨进程沙盒化工具调用以及领域模板市场。
