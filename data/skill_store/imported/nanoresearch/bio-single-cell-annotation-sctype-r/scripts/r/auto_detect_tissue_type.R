...
auto_detect_tissue_type <- function(path_to_db_file, seuratObject, scaled, assay = "RNA", ...){
    ...
    for(tissue in tissues_){ print(paste0("Checking...", tissue));
        ...
        # Reliable Seurat v5 detection (consistent with sctype_annotation.R)
        seurat_v5 <- inherits(seuratObject[[assay]], "Assay5")
        data_type <- if (scaled) "scale.data" else "counts"
        obj <- if (seurat_v5) {
          as.matrix(Seurat::GetAssayData(seuratObject, assay = assay, layer = data_type))
        } else {
          as.matrix(Seurat::GetAssayData(seuratObject, assay = assay, slot = data_type))
        }
        ...
        dt_out = cL_resutls %>% dplyr::group_by(cluster) %>% dplyr::slice_max(order_by = scores, n = 1, with_ties = FALSE)
