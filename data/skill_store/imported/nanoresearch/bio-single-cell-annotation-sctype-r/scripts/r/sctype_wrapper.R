...
sctype_source <- function(){
    # Pin to a specific commit for reproducibility and stability.
    # If raw URLs are unavailable, fall back to local copies in assets/.
    commit <- "630e15cf1e51f2612eda4ad0406dfb17503fa8c9"  # master as of 2025-05
    base_url <- paste0("https://raw.githubusercontent.com/IanevskiAleksandr/sc-type/", commit)
    
    # Try pinned commit first, fall back to master, then local assets
    source_scripts <- function(url, fallback_url, local_path) {
      tryCatch({
        source(url)
      }, error = function(e1) {
        tryCatch({
          ...
          if (file.exists(local_path)) {
            ...
            stop(sprintf("Cannot source %s from remote or local assets/", basename(local_path)))
          }
        })
      })
    }
...
run_sctype <- function(seurat_object, known_tissue_type = NULL, assay = "RNA", scaled = TRUE, custom_marker_file = NULL, plot = FALSE, name = "sctype_classification") {
    ...
    # Reliable Seurat v5 detection
    seurat_v5 <- inherits(seurat_object[[assay]], "Assay5")
    if (seurat_v5) {
        print("Using Seurat v5 object")
        if (!requireNamespace("SeuratObject", quietly = TRUE)) {
            stop("Please install SeuratObject: install.packages('SeuratObject')")
        }
        scRNAseqData <- as.matrix(SeuratObject::LayerData(
            seurat_object, assay = assay, layer = data_type
        ))
    } else {
        print("Using Seurat v4 object")
        scRNAseqData <- as.matrix(Seurat::GetAssayData(
            seurat_object, assay = assay, slot = data_type
        ))
    }
    ...
    sctype_scores = cL_resutls %>% group_by(cluster) %>% slice_max(order_by = scores, n = 1, with_ties = FALSE)
