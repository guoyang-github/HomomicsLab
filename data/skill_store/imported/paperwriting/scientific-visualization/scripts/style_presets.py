...
def apply_publication_style(style_name: str = 'default') -> None:
    ...
    style_name : str, default 'default'
        ...
        - 'science_china': Science China / Chinese Science Bulletin style (with Chinese font support)
        - 'acta_physica': Acta Physica Sinica style (with Chinese font support)
        - 'journal_computers': Journal of Computers (计算机学报) style (with Chinese font support)
        - 'chinese_thesis': Chinese thesis/dissertation style (with Chinese font support)
    ...
    >>> apply_publication_style('science_china')
    >>> fig, ax = plt.subplots()
    >>> ax.plot([1, 2, 3], [1, 4, 9])
    """
    ...
    elif style_name == 'science_china':
        base_style.update({
            'font.size': 8,
            'font.family': 'sans-serif',
            'font.sans-serif': ['SimHei', 'Source Han Sans CN', 'Noto Sans CJK SC',
                               'WenQuanYi Micro Hei', 'Arial', 'Helvetica', 'DejaVu Sans'],
            'axes.labelsize': 9,
            'axes.titlesize': 9,
            'xtick.labelsize': 8,
            'ytick.labelsize': 8,
    ...
    elif style_name == 'journal_computers':
        base_style.update({
            'font.size': 9,
            'font.family': 'sans-serif',
            'font.sans-serif': ['SimHei', 'Source Han Sans CN', 'Noto Sans CJK SC',
    ...
    elif style_name == 'chinese_thesis':
        base_style.update({
            'font.size': 10,
            'font.family': 'serif',
            'font.serif': ['SimSun', 'Source Han Serif CN', 'Noto Serif CJK SC',
                          'WenQuanYi Micro Hei', 'Times New Roman', 'DejaVu Serif'],
            'font.sans-serif': ['SimHei', 'Source Han Sans CN', 'Noto Sans CJK SC',
                               'WenQuanYi Micro Hei', 'Arial', 'Helvetica', 'DejaVu Sans'],
            'axes.labelsize': 11,
            'axes.titlesize': 11,
...
def configure_for_journal(journal: str, figure_width: str = 'single') -> None:
    ...
    ----------
    journal : str
        Journal name. U.S. journals: 'nature', 'science', 'cell', 'plos', 'acs', 'ieee'.
        Chinese journals: 'science_china', 'sci_bulletin', 'acta_physica',
        'journal_computers', 'chinese_thesis'.
    ...
    >>> fig, ax = plt.subplots()  # Configured for Science China
    >>> ax.set_xlabel('Time (s)', fontproperties=chinese_font)
    """
    ...
    journal_configs = {
        ...
        'journal_computers': {
            'single_width': 80,
            'double_width': 160,
            'style': 'journal_computers',
        },
        'chinese_thesis': {
            'single_width': 140,  # A4 page, larger figures
            'double_width': 160,
            'style': 'chinese_thesis',
        },
    }
