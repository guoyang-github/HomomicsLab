...
**Note: This skill contains no automation scripts. It is a pure knowledge/reference skill providing structured evaluation checklists and criteria.**
...
- Reporting only relative risk reduction without absolute risk reduction (ARR, NNT)
- p < 0.05 with trivial effect size and wide confidence intervals in very large samples
- Subgroup analyses without formal tests of interaction (post-hoc subgroups reported as confirmatory)
- Surrogate endpoints presented as equivalent to clinical endpoints without validation
- Claims of "significance" without reporting effect sizes and confidence intervals
...
- **Effect measure appropriateness**: Is RR reported for cohort/RCT, OR for case-control/logistic, HR for survival? Is ARR/NNT provided for clinical decision-making?
- **P-value interpretation**: Is p < 0.05 interpreted as the probability of data under the null (correct) or as the probability the null is true / the finding is "real" (incorrect)?
- **Clinical significance**: With large N, trivial effects become statistically significant. Does the author assess whether the effect size is clinically/biologically meaningful?

**Evidence Quality Assessment (GRADE):**
- **Starting certainty**: RCTs → High; Observational studies → Low
- **Downgrade for**: Risk of bias, inconsistency (heterogeneity), indirectness (different population/outcome), imprecision (wide CI), publication bias
- **Upgrade for**: Large effect (OR > 5), dose-response relationship, all plausible confounders would reduce the effect
- **Final rating**: High / Moderate / Low / Very Low — does the manuscript appropriately qualify its conclusions by evidence certainty?
- **Study design appropriateness**: Does the design match the question? (e.g., RCT for intervention, cross-sectional for diagnostic accuracy, cohort for prognosis)
...
- **scientific-manuscript** — Use as a self-review tool before submitting your own paper; the checklists and criteria help identify weaknesses early
- **scientific-research-design** — The methodological and statistical criteria applied here should inform how you design your own studies
- **scientific-grant-writing** — Grant proposals can be evaluated using the same rigor frameworks; mock peer review strengthens applications
- **scientific-visualization** — Figure quality checks (resolution, labeling, accessibility) apply equally to reviewing others' work and preparing your own
- **scientific-illustration** — Diagram clarity and scientific accuracy standards guide both critique and creation of visual elements
