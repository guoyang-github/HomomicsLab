...
First, the assistant should gather information about the user's project from Skill 1:
...
the assistant should work with the user to list EVERY assumption the project makes from inception through conclusion. Assumptions fall into two categories:
...
For each assumption, the assistant should help the user assign two scores:
...
Once the complete table is ready, the assistant should analyze the risk profile:
...
For each high-risk assumption (level 4-5), the assistant should help develop mitigation strategies:
...
For the top 3 highest-risk assumptions, the assistant should help design the critical go/no-go experiments:
...
the assistant should search PubMed to help calibrate risk assessments:
...
Based on the risk analysis, the assistant should help create a revised plan:
...
the assistant should produce a **2-page Risk Assessment Document**:
...
the assistant should begin with Phase 1 by asking for:
1. The project summary from Skill 1
2. Project timeline expectations
3. What concerns the user most about this project

Together, the assistant and the user will build a rigorous risk assessment that dramatically improves the likelihood of success by helping avoid years of work on projects with insurmountable obstacles.
