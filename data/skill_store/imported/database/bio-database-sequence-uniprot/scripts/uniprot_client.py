...
def map_ids(ids: List[str], from_db: str, to_db: str,
           format: str = "json", poll_interval: Optional[int] = None,
    ...
    Args:
        ...
        format: Response format
        poll_interval: Seconds between status polls (default: module POLLING_INTERVAL)
        max_polls: Maximum number of status polls (default: 100)
    ...
    interval = poll_interval if poll_interval is not None else POLLING_INTERVAL
    polls = max_polls if max_polls is not None else 100
    ...
    response = requests.post(submit_endpoint, data=data, timeout=30)
    response.raise_for_status()
    ...
    response = requests.get(results_endpoint, params=params, timeout=30)
    response.raise_for_status()
