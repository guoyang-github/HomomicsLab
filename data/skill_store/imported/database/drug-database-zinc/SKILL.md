...
**API endpoint**:
```bash
curl "https://cartblanche22.docking.org/substances.txt:zinc_id=ZINC000000000001&output_fields=smiles,zinc_id"
```
...
**API endpoint**:
```bash
curl "https://cartblanche22.docking.org/smiles.txt:smiles=c1ccccc1&dist=4"
```
...
def query_zinc_by_id(zinc_id, output_fields="zinc_id,smiles,catalogs"):
    ...
    url = (
        ...
        f":zinc_id={urllib.parse.quote(zinc_id)}"
        f"&output_fields={urllib.parse.quote(output_fields)}"
    )
...
def search_by_smiles(smiles, dist=0, adist=0, output_fields="zinc_id,smiles"):
    ...
    url = (
        f"https://cartblanche22.docking.org/smiles.txt"
        f":smiles={urllib.parse.quote(smiles)}"
        f"&dist={dist}&adist={adist}"
        f"&output_fields={urllib.parse.quote(output_fields)}"
    )
    response = requests.get(url, timeout=30)
    response.raise_for_status()
...
def get_random_compounds(count=100, subset=None, output_fields="zinc_id,smiles,tranche"):
    ...
    url = (
        f"https://cartblanche22.docking.org/substance/random.txt"
        f":count={count}&output_fields={urllib.parse.quote(output_fields)}"
    )
    if subset:
        url += f"&subset={urllib.parse.quote(subset)}"
    response = requests.get(url, timeout=30)
    response.raise_for_status()
    return response.text
```
