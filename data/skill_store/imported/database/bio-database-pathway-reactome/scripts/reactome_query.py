...
class ReactomeClient:
    ...
    def get_version(self) -> str:
        ...
        response = requests.get(f"{self.CONTENT_BASE}/data/database/version", timeout=30)
        response.raise_for_status()
        return response.text.strip()

    def query_pathway(self, pathway_id: str) -> Dict:
        """Query pathway information by ID"""
        response = requests.get(f"{self.CONTENT_BASE}/data/query/{pathway_id}", timeout=30)
        response.raise_for_status()
    ...
    def search_pathways(self, term: str) -> List[Dict]:
        ...
        response = requests.get(
            ...
            params={"query": term, "species": "Homo sapiens"},
            timeout=30
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_analysis_by_token(self, token: str) -> Dict:
        ...
        response = requests.get(f"{self.ANALYSIS_BASE}/token/{token}", timeout=30)
        response.raise_for_status()
        return response.json()
