...
- Navigation is hash-routed (`App.tsx` syncs `activeItem` with `window.location.hash`); no react-router. Sidebar shows only Chat / Files / Skills / Domains / MCP / Settings, plus a Project selector and session history (`components/layout/SidebarSessions.tsx`) below the main nav. Reports / Workflow / Figures are not top-level entries; they surface as inline cards in the message stream and open full-screen overlays via `src/components/overlay/OverlayManager.tsx`.
- Message types: `frontend/src/types/chat.ts` defines the chat message contract. The frontend supports `type: 'artifact'` for rendering reports / figures / tables / images / html / json / anndata inline (`components/chat/ArtifactMessage.tsx`); the backend enum `MessageType` currently does not emit `artifact`, so enabling it requires a backend contract change. Execution progress is shown by `components/chat/ExecutionProgressBar.tsx` at the top of the chat, not as a large inline message card.
