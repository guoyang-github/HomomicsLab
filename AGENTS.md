...
- Navigation is hash-routed (`App.tsx` syncs `activeItem` with `window.location.hash`); no react-router. Sidebar shows only Chat / Files / Skills / Domains / MCP / Settings. The Project selector sits below the nav and directly above session history (`components/layout/SidebarSessions.tsx`) so project/session context is grouped; labels are title-case and the project select is compact. In collapsed mode the sidebar header shows only the centered logo; the collapse toggle lives in a small footer. Reports / Workflow / Figures are not top-level entries; they surface as inline cards in the message stream and open full-screen overlays via `src/components/overlay/OverlayManager.tsx`.
- Message types: `frontend/src/types/chat.ts` defines the chat message contract. Artifacts from successful runs travel **embedded** in the `todo_list` message content (`TodoListContent.artifacts` / `result.artifacts`) and are rendered inline by `components/chat/ExecutionResult.tsx`; `models/common.py` `MessageType.ARTIFACT` still exists as an enum value but the backend never constructs `type: 'artifact'` chat messages, so `MessageBubble.tsx` carries no artifact branch. The backend also recognizes `analysis_type: 'visualization_edit'` (`agent/intent/classifiers.py`) for natural-language figure edits; `turn_responder.py` (`VisualizationEditHandler`, re-exported via the `turn_viz_handler.py` shim) + `viz/edit_engine.py` apply rule-based Plotly transforms and return a new `PLOT_DATA` message.
- Execution progress is shown by `components/chat/TodoChecklist.tsx` as a compact checklist above the chat input, not as a top progress bar or large inline card. The send button in `components/chat/ChatInput.tsx` switches to an abort/stop state while a run is in progress. The top bar (`components/layout/TopBar.tsx`) is compact (`h-10`), right-aligned, and opens the command palette via an icon-only button alongside theme/user controls; the chat panel has no redundant second header, keeping the visual focus on the conversation.
- Workflow (`components/workspace/FlowCanvas.tsx`, `components/workspace/Workspace.tsx`) is a read-only plan/audit visualization and provenance view; it does not duplicate execution controls (run/retry/skip/abort live in the chat input) and does not embed data upload (data management belongs in Files). For domain tasks (domain != generic) FlowCanvas switches to the domain pipeline view: the backend emits a one-shot `workflow_skeleton` progress event plus per-phase `phase` events (full `ExecutionState` dicts with extra top-level keys on the execution SSE `state` channel; see `agent/workflow_markers.py`), stored per job as `JobRuntime.workflowSkeleton` / `phaseStates` (`stores/executionStore.ts`) and wired in `hooks/useExecutionSSE.ts`. On the CodeAct path, phase markers stream in near real time via the sandbox `run_command(..., on_output_line=...)` line callback (all three backends; a per-task `(phase, status)` dedupe set in `agent/orchestrator_executors.py` keeps the post-execution batch scan from re-emitting), and the fixed_pipeline curated-runtime dispatch emits the same skeleton plus task-state→phase events. Nodes are the non-skipped skeleton phases chained serially; the "View full workflow" entry in TodoChecklist hides for positively-generic tasks (`utils/workflowPhases.ts` `resolveWorkflowAvailability`), and without a skeleton the canvas keeps the legacy taskStore/planStore rendering (including `display_subtasks` expansion).
...
- Self-improvement data:
  ...
  - `skills/genesis.py` (`SkillGenesis`, always on) crystallizes repeatedly-validated CodeAct scripts into community-trust skills: candidates are a post-self-correction success or `SKILL_GENESIS_MIN_SUCCESSES` (constant in `skills/genesis.py`, 3) successes of one normalized task signature, tracked as namespaced rows (`genesis:<hash>`) in the CBKB `parameter_lore` table (no new table). Candidate packages are staged under `data/skill_genesis/`, then **registered directly as `community`-trust skills** — genesis is notification-based, not approval-based: a notification ("learned a new skill X from N successful runs") is emitted through the injected `notify` channel, there is no HITL approval state machine, and the undo path is the existing skill deletion API (`DELETE /api/skills/{skill_id}`). A signature is crystallized at most once. The hook lives in `agent/turn_feedback_recorder.py` (CodeAct results are recognized by their `code` key); DAG linking uses the standard `propose_edge` CANDIDATE path — never bypass `skills/promotion.py`/`trust_skill` for trust changes.
...
`SKILL.md` follows a minimal, ecosystem-compatible contract (aligned with agentskills.org):

- **Required**: `name`, `description`. A missing `name` is inferred from the directory name; a missing `description` only logs a warning.
- **Recommended**: `tool_type`, `keywords`, `inputs`, `outputs`. When `tool_type` is omitted it is inferred from `scripts/` (python/r/mixed; no scripts → agent/knowledge):
...
- **Optional**: `requirements.txt`, `environment.yml`, `assets/`, `trust_level`, and any custom frontmatter fields (ignored with a debug log).

Validate skills with `homomics skill validate <path>` (errors for missing required fields, warnings for missing recommended ones; `--strict` turns warnings into a non-zero exit).
...
- **Scheduled background jobs** (`curation`, `evolution`, `narrative reports`, `SOP proposals`) are always registered ("default awake") — there is no enable/disable switch. Each run is gated by data volume: the job executes only when the CBKB holds at least `MIN_EXPERIMENTS_*` experiment nodes (`scheduler.py`: curation 10, evolution 10, SOP proposal 5, narrative 3); below the threshold the run is skipped (log-only).
...
- `agent/turn_runner.py` is no longer a god class; it delegates to five consolidated collaborator modules: `turn_intent_router` (routing), `turn_executor` (workflow / agent-loop / tool-approval orchestration), `turn_responder` (response generation, result assembly, clarification, visualization edits), `turn_state` (turn persistence, context formatting, file resolution), and `turn_guard` (risk assessment, self-correction), plus the constructor-injected `turn_feedback_recorder`. Collaborators receive dependencies via constructor injection (providers for lazily-built services) and per-turn mutable state via an explicit `ctx` dict — no runner back-references outside `turn_intent_router`. Do not add new large methods directly to `turn_runner.py` — place them in the appropriate collaborator module and add a thin delegating method only when necessary.
