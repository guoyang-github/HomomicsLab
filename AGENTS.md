...
- Navigation is hash-routed (`App.tsx` syncs `activeItem` with `window.location.hash`); no react-router. Sidebar shows only Chat / Files / Skills / Domains / MCP / Settings. The Project selector sits below the nav and directly above session history (`components/layout/SidebarSessions.tsx`) so project/session context is grouped. The sidebar collapse toggle stays inside the header and keeps icons centered. Reports / Workflow / Figures are not top-level entries; they surface as inline cards in the message stream and open full-screen overlays via `src/components/overlay/OverlayManager.tsx`.
