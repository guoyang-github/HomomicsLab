...
- Workflow (`components/workspace/FlowCanvas.tsx`, `components/workspace/Workspace.tsx`) is a read-only plan/audit visualization and provenance view; it does not duplicate execution controls (run/retry/skip/abort live in the chat input) and does not embed data upload (data management belongs in Files).
