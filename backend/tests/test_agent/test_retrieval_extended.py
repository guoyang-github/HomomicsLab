...
def test_retrieve_tools_by_description():
...
def test_retrieve_data_sources():
    ...
    ctx = retriever.retrieve("PBMC single cell analysis", "single_cell_analysis")

    assert any(d.id == "pbmc3k" for d in ctx.data_sources)
    assert not any(d.id == "gtex" for d in ctx.data_sources)


def test_prompt_context_includes_tools_and_data_sources():
    registry = SkillRegistry()
    tool_registry = ToolRegistry()
    tool_registry.register_builtin(
        name="pubmed_search",
    ...
    ctx = retriever.retrieve("single cell PBMC paper analysis", "single_cell_analysis")
...
def test_override_data_sources_per_retrieve():
    registry = SkillRegistry()
    default_sources = [
    ...
    ctx = retriever.retrieve("override analysis", "test", data_sources=override_sources)
