---
name: bio-single-cell-markers
description: Find marker genes and manually assign cell type labels in single-cell RNA-seq using Seurat (R) and Scanpy (Python). Use for differential expression between clusters, identifying cluster-specific markers, scoring gene sets, and assigning cell type labels. Use when finding marker genes and annotating clusters.
tool_type: mixed
primary_tool: Seurat
supported_tools: [scanpy, matplotlib, pandas]
keywords: ["single-cell", "marker-genes", "differential-expression", "cell-type-annotation", "Seurat", "Scanpy", "findallmarkers", "parallel-execution", "future", "multicore"]
---
...
| Method | Language | Test Type | Best For | Key Features |
|--------|----------|-----------|----------|--------------|
| **Wilcoxon** | Python/R | Non-parametric | General marker detection | Default, robust |
| **t-test** | Python/R | Parametric | Large samples | Fast, assumes normality |
| **MAST** | R | Hurdle model | Sparse data | Handles dropouts |
| **DESeq2** | R | Negative binomial | Raw counts | Differential expression |
| **Logistic Regression** | Python/R | Classification | Cluster separation | Alternative view |

### Selection Guide

| Your Situation | Recommended Method | Why |
|----------------|-------------------|-----|
| Standard analysis | Wilcoxon | Default, well-tested |
| Raw count data | DESeq2 | Designed for counts |
| Sparse dropout data | MAST | Hurdle model handles zeros |
| Python workflow | Scanpy wilcoxon | Native implementation |
| R workflow | Seurat Wilcoxon | Best integration |
...
|-------|-------------|------------|
| [Normalized] | Log-normalized data | Input for marker detection |
| [Clustered] | Cluster labels assigned | Required for group-wise markers |
| [Annotated] | Cell type labels | Output after manual annotation |
...
**Input State:** [Normalized] + [Clustered]
**Output State:** [Annotated] (markers identified, cell types assigned)
...
- Dataset has >10 clusters (highly recommended)
- Large cell count (>10,000 cells)
- Using slower statistical tests (MAST, DESeq2)
- Batch processing multiple samples
- System has 4+ CPU cores available

**When NOT to use parallel:**
- Small datasets (<5 clusters, <5,000 cells) - overhead exceeds benefit
- Memory-constrained systems (each worker uses additional RAM)
- Quick exploratory analysis
- Windows systems with limited RAM (multisession uses more memory)
...
# Recommended: specify workers based on system
all_markers <- FindAllMarkersParallel(
    seurat_obj,
    n_workers = 4,              # Use 4 parallel workers
    future.plan = 'multicore',  # Linux/Mac: shared memory
    only.pos = TRUE,
    min.pct = 0.25
)
...
# Parallel markers for specific clusters
...
benchmark <- BenchmarkParallelMarkers(seurat_obj, n_workers = c(1, 2, 4, 8))
```

**Performance considerations:**
- Each worker loads a copy of the data - monitor RAM usage
- Recommended workers: `detectCores() - 1`
- Diminishing returns after 8-16 workers
- Multicore plan (Linux/Mac) is faster than multisession (Windows)
...
- bio-single-cell-clustering - Must cluster before finding markers
- bio-single-cell-preprocessing - Data must be normalized
- bio-single-cell-data-io - Export annotated data
