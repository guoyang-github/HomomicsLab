...
if (!dir.exists(paneldir)) {
    stop('Phasing reference panel directory not found')
}
...
if (bulk) {
    ...
    for (i in 1:n_samples) {
        ...
        cmd = glue(
            'cellsnp-lite',
            '-S {shQuote(bam_file)}',
            '-i {shQuote(sample_file)}',
            '-O {shQuote(glue("{outdir}/pileup/{samples[i]}"))}',
            '-R {shQuote(snpvcf)}',
            '-p {ncores}',
            '--minMAF 0',
            '--minCOUNT 2',
    ...
    cmd = glue(
            ...
            '-S {shQuote(bams)}',
            '-i {shQuote(barcodes)}',
            '-O {shQuote(glue("{outdir}/pileup/{samples}"))}',
            '-R {shQuote(snpvcf)}',
            '-p {ncores}',
            '--minMAF 0',
            '--minCOUNT 2',
...
system(glue('sh {shQuote(script)} 2>&1 | tee {shQuote(glue("{outdir}/pileup.log"))}'), intern = FALSE)
...
# NOTE: genotype() is an internal Numbat function (not exported).
# If Numbat exports this in a future version, replace numbat::: with numbat::
# Try exported version first (Numbat v2+), fallback to internal
numbat_version <- tryCatch(packageVersion("numbat"), error = function(e) NULL)
if (!is.null(numbat_version)) {
    message(paste0("Detected Numbat version: ", numbat_version))
}

genotype_fn <- tryCatch({
    get("genotype", envir = asNamespace("numbat"), inherits = FALSE)
}, error = function(e) {
    stop(paste(
        "Cannot access Numbat's genotype() function.",
        "This function is internal and may have changed or been removed in your Numbat version.",
        "Please verify your Numbat installation:",
        "  remotes::install_github('kharchenkolab/numbat', force = TRUE)",
        "If the issue persists, check the Numbat GitHub repository for API changes."
    ))
})

tryCatch({
    genotype_fn(label, samples, vcfs, glue('{outdir}/phasing'), chr_prefix = TRUE)
}, error = function(e) {
    stop(paste(
        "genotype() failed:", e$message,
        "\nThis may be due to Numbat version incompatibility.",
        "The genotype() function is internal to Numbat and may change between versions.",
        "\nCurrent Numbat version:", as.character(numbat_version),
        "\nSuggested fix: reinstall Numbat from GitHub:",
        "  remotes::install_github('kharchenkolab/numbat', force = TRUE)"
    ))
})
...
eagle_cmd = function(chr) {
    paste(shQuote(eagle),
        ...
        glue('--vcfTarget {shQuote(glue("{outdir}/phasing/{label}_chr{chr}.vcf.gz"))}'),
        glue('--vcfRef {shQuote(glue("{paneldir}/chr{chr}.genotypes.bcf"))}'),
        glue('--geneticMapFile={shQuote(gmap)}'),
        glue('--outPrefix {shQuote(glue("{outdir}/phasing/{label}_chr{chr}.phased"))}'),
    sep = ' ')
}
...
phasing_status <- system2(script, stdout = glue("{outdir}/phasing.log"))
if (phasing_status != 0) {
    stop(glue('Phasing failed with exit code {phasing_status}. Check {outdir}/phasing.log for details.'))
}
...
for (sample in samples) {
    ...
    # NOTE: preprocess_allele() is an internal Numbat function (not exported).
    # If Numbat exports this in a future version, replace numbat::: with numbat::
    # Try exported version first, fallback to internal with error handling
    preprocess_allele_fn <- tryCatch({
        get("preprocess_allele", envir = asNamespace("numbat"), inherits = FALSE)
    }, error = function(e) {
        stop(paste(
            "Cannot access Numbat's preprocess_allele() function.",
            "This function is internal and may have changed or been removed in your Numbat version.",
            "Please verify your Numbat installation:",
            "  remotes::install_github('kharchenkolab/numbat', force = TRUE)",
            "If the issue persists, check the Numbat GitHub repository for API changes."
    ...
    df <- tryCatch({
        ...
        stop(paste(
            "preprocess_allele() failed:", e$message,
            "\nThis may be due to Numbat version incompatibility.",
            "The preprocess_allele() function is internal to Numbat and may change between versions.",
            "\nCurrent Numbat version:", as.character(numbat_version),
            "\nSuggested fix: reinstall Numbat from GitHub:",
            "  remotes::install_github('kharchenkolab/numbat', force = TRUE)"
        ))
