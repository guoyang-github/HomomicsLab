...
tool_type: mixed
primary_tool: Seurat
supported_tools: [scanpy, matplotlib, numpy, pandas]
keywords: ["single-cell", "preprocessing", "QC", "normalization", "filtering", "HVG", "quality-control"]
---

## Version Compatibility

Reference examples tested with: Seurat 4.x/5.x+, ggplot2 3.5+, matplotlib 3.8+, numpy 1.26+, scanpy 1.10+
...
- Quality control metrics calculation (mitochondrial %, gene counts)
- Filtering cells and genes based on QC thresholds
- Normalization (log, SCTransform)
- Highly variable gene selection
- Scaling and regression

### What This Skill Does NOT Cover
- **Data loading** → Use [bio-single-cell-data-io](../bio-single-cell-data-io/)
- **Dimensionality reduction** → Use [bio-single-cell-clustering](../bio-single-cell-clustering/)
- **Clustering** → Use [bio-single-cell-clustering](../bio-single-cell-clustering/)

### Workflow Position

```
[bio-single-cell-data-io] → [bio-single-cell-preprocessing] → [bio-single-cell-clustering]
        Load data          →    QC, Filter, Normalize      →    PCA, Cluster, UMAP
```
...
|-------|------|-----|-------------|
| [Raw] | data-io | - | Raw count matrix |
| [QC] | preprocessing | preprocessing | Calculated QC metrics |
| [Filtered] | preprocessing | preprocessing | After cell/gene filtering |
| [Normalized] | preprocessing | preprocessing | After normalization |
| [HVG] | preprocessing | clustering | Variable gene selection |
| [Scaled] | preprocessing | clustering | Ready for PCA |
...
# - pct_counts_ribo: percentage ribosomal
```
...
# Option 1: Use 'seurat' flavor on log-transformed data (most common)
sc.pp.highly_variable_genes(adata, n_top_genes=2000, flavor='seurat')

# Option 2: Use 'seurat_v3' flavor on raw counts (before normalization)
# This requires the 'counts' layer to be present
...
> **Seurat v5 note:** In v5, expression data is stored in `@assays$RNA@layers$counts` (raw), `@layers$data` (normalized), and `@layers$scale.data` (scaled). The high-level functions below work the same in v4 and v5, but direct slot access differs.
...
### Cell Cycle Scoring (Seurat)

```r
# Score cells for cell cycle phase
# Uses built-in cell cycle gene lists for human
seurat_obj <- CellCycleScoring(seurat_obj,
    s.features = cc.genes.updated.2019$s.genes,
    g2m.features = cc.genes.updated.2019$g2m.genes,
    set.ident = FALSE)

# View cell cycle distribution
VlnPlot(seurat_obj, features = c('S.Score', 'G2M.Score'))

# Regress out cell cycle during scaling if needed
...
# For Seurat v5, vst.flavor = 'v2' is recommended for improved performance
seurat_obj <- SCTransform(seurat_obj, vars.to.regress = 'percent.mt', vst.flavor = 'v2', verbose = FALSE)
```
...
- bio-single-cell-data-io - Load data before preprocessing
- bio-single-cell-clustering - PCA and clustering after preprocessing
- bio-single-cell-cell-annotation - Find markers after clustering
