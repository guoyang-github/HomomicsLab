...
class RateLimiter:
    ...
    def wait_if_needed(self):
        ...
        with self._lock:
            ...
            # Check if at limit
            if len(self._requests) >= self.max_per_minute:
                sleep_time = 60 - (now - self._requests[0]) + 0.1
                if sleep_time > 0:
                    ...
                    try:
                        ...
                        self._lock.acquire()
                    # Re-check and clean up after sleep
                    now = time.time()
                    while self._requests and now - self._requests[0] > 60:
                        self._requests.popleft()
...
class FDAQuery:
    ...
    def _make_request(self, url: str, params: Dict, use_cache: bool = True) -> Dict:
        ...
        Returns:
            API response as dictionary (always contains 'error' or 'results')
        """
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                return {"error": f"Invalid query: {response.text}", "results": []}
            else:
                return {"error": f"HTTP error {response.status_code}: {e}", "results": []}
        except json.JSONDecodeError as e:
            return {"error": f"Invalid JSON response: {e}", "results": []}
