...
def _fetch_text(url: str, data: Optional[bytes] = None,
                expected_content_type: Optional[str] = None) -> str:
    """
    Fetch text data from STRING API with validation.

    Args:
        url: Full API URL
        data: POST data (optional)
        expected_content_type: Expected content type substring (e.g., 'text/plain')

    Returns:
        Response text, or error string starting with "Error:" on failure
    """
    try:
        req = urllib.request.Request(url, data=data,
                                     headers={"User-Agent": "string-api-client/1.0"})
        ...
        return f"Error: Cannot decode response as UTF-8 - {e}"


def _fetch_bytes(url: str, expected_content_type: str = "image") -> bytes:
    """
    Fetch binary data from STRING API with validation.

    Args:
        url: Full API URL
        expected_content_type: Expected content type substring (e.g., 'image/png')

    Returns:
        Response bytes, or empty bytes on failure (caller should check len())
    """
    try:
...
def _format_identifiers(identifiers: Union[str, List[str]],
                        joiner: str = '%0d') -> str:
    """Format identifier(s) for URL parameters."""
    if isinstance(identifiers, list):
        return joiner.join(identifiers)
    return identifiers
...
def string_map_ids(identifiers: Union[str, List[str]],
    ...
    Returns:
        str: TSV format with mapping results, or error string on failure
    ...
    return _fetch_text(url, data=data, expected_content_type="text")
...
def string_network(identifiers: Union[str, List[str]],
    ...
    Returns:
        str: TSV format with interaction data, or error string on failure
    ...
    """
    identifiers_str = _format_identifiers(identifiers)
    ...
    return _fetch_text(url, expected_content_type="text")
...
def string_network_image(identifiers: Union[str, List[str]],
    ...
    return _fetch_bytes(url, expected_content_type="image")
...
def string_interaction_partners(identifiers: Union[str, List[str]],
    ...
    Returns:
        str: TSV format with interaction partners, or error string on failure
...
def string_enrichment(identifiers: Union[str, List[str]],
    ...
    Returns:
        str: TSV format with enrichment results, or error string on failure
...
def string_ppi_enrichment(identifiers: Union[str, List[str]],
    ...
    Returns:
        str: JSON with PPI enrichment p-value, or error string on failure
    ...
    return _fetch_text(url, expected_content_type="application/json")
...
def string_homology(identifiers: Union[str, List[str]],
    ...
    Returns:
        str: TSV format with homology scores, or error string on failure
...
def string_version() -> str:
    ...
    Returns:
        str: Version information, or error string on failure
...
if __name__ == "__main__":
    ...
    if network.startswith("Error:"):
        print(f"Failed: {network}")
    else:
        print(network[:500] + "...")
