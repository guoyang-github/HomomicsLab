...
# Reference: myvariant 1.0+, pandas 2.2+ | Verify API if version differs
...
def get_gnomad_frequencies(variant_hgvs):
    ...
    exome = (result.get('gnomad_exome') or {}).get('af', {})
    genome = (result.get('gnomad_genome') or {}).get('af', {})
...
if __name__ == '__main__':
    ...
    for v in variants:
        ...
        if freqs:
            ...
            max_af = max((v for v in [exome_af, genome_af] if v is not None), default=None)
            print(f'{v}:')
            print(f'  Exome AF: {exome_af if exome_af is not None else "absent"}')
            print(f'  Genome AF: {genome_af if genome_af is not None else "absent"}')
            print(f'  Max AF: {max_af if max_af is not None else "absent"}')
        else:
    ...
    for r in records:
        max_af = max((v for v in [r['exome_af'], r['genome_af']] if v is not None), default=None)
        rare = is_rare(max_af)
