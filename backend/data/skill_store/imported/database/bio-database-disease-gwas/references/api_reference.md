...
- New endpoints (genomic context, ancestries)
...
# Find all associations for type 2 diabetes (REST API uses MONDO IDs)
...
> **Note:** `findByTraitIgnoreCase` endpoint is not available in API v2.0. Use the list endpoint (`GET /efoTraits`) and filter client-side, or search via the web interface.

**Example:**
```python
import requests

# Get trait details (REST API uses MONDO IDs)
trait_id = "MONDO_0005148"
...
print(f"URI: {trait.get('uri')}")
```

> **Note:** The `publications` and `genes` endpoints documented in earlier API versions are not available in API v2.0. Publication information is available via the `/studies` endpoint (`publicationInfo` field). Gene-related variants can be queried via `/singleNucleotidePolymorphisms/search/findByGene`.
...
response = requests.get("https://www.ebi.ac.uk/gwas/rest/api/studies/GCST001795", timeout=10)
response.raise_for_status()
study = response.json()
...
# Note: Summary Statistics API uses EFO IDs
...
```
GET /chromosomes/{chromosome}/associations?bp_start={start}&bp_end={end}
```
...
def get_complete_variant_data(rs_id):
    ...
    response = requests.get(main_url, headers={"Content-Type": "application/json"}, timeout=10)
    response.raise_for_status()
    variant_info = response.json()

    # Get associations
    assoc_url = f"{main_url}/associations"
    response = requests.get(assoc_url, headers={"Content-Type": "application/json"}, timeout=10)
    response.raise_for_status()
...
def query_region(chromosome, start, end, p_threshold=None):
    ...
    sumstats_params = {"bp_start": start, "bp_end": end, "size": 1000}
    if p_threshold:
        sumstats_params["p_upper"] = str(p_threshold)

    sumstats_response = requests.get(sumstats_url, params=sumstats_params, timeout=10)
...
# Query type 2 diabetes associations (REST API uses MONDO IDs)
df = gwas.get_trait_associations('MONDO_0005148')
...
def download_summary_statistics(gcst_id, output_dir="."):
    ...
    try:
        response = requests.get(harmonised_url, stream=True, timeout=10)
        response.raise_for_status()
