---
name: bio-database-disease-gwas
...
- Trait IDs: MONDO IDs for REST API (e.g., `MONDO_0005148` for type 2 diabetes mellitus), EFO IDs for Summary Statistics API (e.g., `EFO_0001360`)
...
**Important: Trait ID Format Differences**
- GWAS Catalog REST API uses **MONDO** IDs (e.g., `MONDO_0005148` for type 2 diabetes mellitus)
- Summary Statistics API uses **EFO** IDs (e.g., `EFO_0001360` for type 2 diabetes)
- Use the `/efoTraits` list endpoint to find the correct ID for your API
...
4. **Traits endpoint** - `/efoTraits/{id}`
   ```python
   # Get trait information (REST API uses MONDO IDs)
...
**Example 1: Find all associations for a disease (REST API)**
```python
import requests

# REST API uses MONDO IDs. Use /efoTraits endpoint to find the correct ID.
trait = "MONDO_0005148"  # type 2 diabetes mellitus
...
**Example 3: Access summary statistics (SumStats API uses EFO IDs)**
```python
import requests

# Query summary statistics API
# Note: Summary Statistics API uses EFO IDs, not MONDO IDs
base_url = "https://www.ebi.ac.uk/gwas/summary-statistics/api"

# Find associations by trait with p-value threshold
trait = "EFO_0001360"  # Type 2 diabetes (EFO ID for SumStats API)
...
response = requests.get("https://www.ebi.ac.uk/gwas/rest/api/studies/GCST001234", timeout=10)
response.raise_for_status()
study = response.json()

# Follow link to associations
associations_url = study['_links']['associations']['href']
associations_response = requests.get(associations_url, timeout=10)
```
...
1. **Identify the trait** using the web interface or API list endpoint:
   - Search web interface for disease name
   - For REST API, note the **MONDO ID** (e.g., `MONDO_0005148` for type 2 diabetes mellitus)
   - For Summary Statistics API, note the **EFO ID** (e.g., `EFO_0001360` for type 2 diabetes)
...
3. **Query via API for specific variants:**
   ...
   params = {"bp_start": start_pos, "bp_end": end_pos}
   ```
...
def query_gwas_catalog(trait_id, p_threshold=5e-8):
    ...
    Args:
        trait_id: MONDO trait identifier for REST API (e.g., 'MONDO_0005148')
                  Use the /efoTraits endpoint to find the correct ID
...
# Example usage
df = query_gwas_catalog('MONDO_0005148')  # type 2 diabetes mellitus (REST API)
print(df.head())
