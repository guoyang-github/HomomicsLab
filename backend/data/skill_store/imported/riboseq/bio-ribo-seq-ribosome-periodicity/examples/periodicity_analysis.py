...
def get_reads_around_start(bam_path, gtf_path, upstream=50, downstream=100):
    '''Extract read positions relative to start codons (genomic coordinate approximation)'''
    # Parse start codon positions from GTF
    starts = []
    with open(gtf_path) as f:
        for line in f:
            ...
            if len(cols) >= 9 and cols[2] == 'start_codon':
                chrom = cols[0]
    ...
    with pysam.AlignmentFile(bam_path, 'rb') as bam:
        for chrom, start, strand in starts:
            try:
                for read in bam.fetch(chrom, max(0, start - upstream), start + downstream):
                    if read.is_unmapped:
                        continue

                    pos_5p = read.reference_end if read.is_reverse else read.reference_start
                    rel_pos = pos_5p - start
                    if strand == '-':
                        rel_pos = start - pos_5p

                    if -upstream <= rel_pos < downstream:
