...
test('CellTypist chat task shows live progress and result', async ({ page }) => {
  ...
  await expect(page.getByText('Automated cell type annotation using CellTypist')).toBeVisible({ timeout: 120_000 })

  // Wait for the running/completed indicator inside the TODO card.
  await expect(page.getByText(/执行中|等待\/已完成|执行完成|执行失败/).first()).toBeVisible({ timeout: 60_000 })

  // Wait for completion or failure card.
  const successCard = page.getByText('执行完成')
  const failureCard = page.getByText('执行失败')
  try {
    await expect(successCard.or(failureCard)).toBeVisible({ timeout: 240_000 })
  } catch (e) {
